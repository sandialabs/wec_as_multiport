...
First, let us consider one-port elements, which, as shown in \figurename~\ref{fig:wec_as_multiport_general_one_two_ports}, relate a single effort ($e$) to a single flow ($q$) through an impedance $Z$.
...
\end{equation}
%
Similarly, two-port elements relate two pairs effort and flow variables (see \figurename~\ref{fig:wec_as_multiport_general_one_two_ports}).\footnote{As the ``multi-port'' name suggests, elements with more than two ports (three, four, etc.) can be considered.}
%
...
Note also the sign conventions illustrated in \figurename~\ref{fig:wec_as_multiport_general_one_two_ports}, which result in power flow into the two-port network from both sides summing to zero for a lossless system.
...
\begin{figure}[tb]
        ...
        \caption{Diagrams and mathematical relationships for transformers, gyrators, and impedances.}
...
% \footnote{Source code to reproduce these results available at \url{https://github.com/ryancoe/wec_as_multiport}} XX
