...
\begin{figure}[tb]
        ...
        \caption{WaveBot device diagram. \rc{XX-note Park transform and modeling boundary} \rc{XX-sign conventions wrong in this diagram}}
...
\begin{subequations}
\begin{gather}
        Z_i v = F_{\textrm{exc}} - F_{u} \label{eq:eom_rectilinear_motion} \\
        Z_i(\omega) = B(\omega) + b_f + j \left( \omega{}(m + A(\omega)) - \frac{k_{h}}{\omega}\right)
\end{gather}
\end{subequations}
...
\figurename~\ref{fig:wec_as_multiport_load_impedance_for_mech_power} uses \eqref{eq:Zl_Zlm} to show the resulting load impedances for two cases:
...
\begin{figure}[tb]
        \centering
        \includegraphics[width=1\columnwidth]{wec_as_multiport_load_impedance_for_mech_power.pdf}
...
\noindent{}From \figurename~\ref{fig:wec_as_multiport_load_impedance_for_mech_power}, we can see that the mechanical maximizing controller looks very different from the controller that maximizes electrical power.
