...
This concept of transferring power between different stages is illustrated in \figurename~\ref{fig:power_transfer_stages}.
...
Because the source power is purely oscillatory, the power flow between the stages in \figurename~\ref{fig:power_transfer_stages} is shown to be bi-directional in general.
In fact, the optimal condition requires that half the power be reflected \rc{XX-cite}.
The conditions that ensure this optimal power transmission by minimizing reflections are shown at the top of \figurename~\ref{fig:power_transfer_stages}.
The first impedance matching condition in \figurename~\ref{fig:power_transfer_stages} ($Z_{\textrm{in}} = Z_i^*$) is generally well-known and widely applied, although using somewhat different notation as the input/output distinction within the PTO is not widely utilized.
The second impedance matching condition in \figurename~\ref{fig:power_transfer_stages} ($Z_{\textrm{out}} = Z_\ell^*$) is mostly ignored and/or lumped with first condition, thus limiting performance.

\begin{figure}[tb]
        \centering
        \includegraphics[width=1\columnwidth]{power_transfer_stages.pdf}
        \caption{XX}
        \label{fig:power_transfer_stages}
\end{figure}
...
% Within this approach to maximize mechanical power, there is an implicit assumption that the dynamics of the PTO can be decoupled from the hydrodynamic problem and, consequently, such a design will also maximize \emph{useful power} at the load (refer back to \figurename~\ref{fig:power_transfer_stages}).
...
We may describe the system with a two-port impedance model, relating the effort ($e_i$) and flow ($q_i$) at each port (see \figurename~\ref{fig:working_paper_general_two_port_network}).
The products of the effort and flow variables on each port is power.
Note also the sign conventions illustrated in \figurename~\ref{fig:working_paper_general_two_port_network}, which result in a convention of positive power flowing into the two-port network from both sides.

\begin{figure}[tb]
        \centering
        \includegraphics[width=0.75\columnwidth]{working_paper_general_two_port_network.pdf}
        \caption{General two-port network definition showing effort ($e_i$) and flow ($q_i$) at each port.}
        \label{fig:working_paper_general_two_port_network}
\end{figure}
...
To this end, let us consider the heaving buoy style ``WaveBot'' device with a direct drive PTO as shown in \figurename~\ref{fig:working_wec_phyiscal_diagram}.
...
\begin{figure}[tb]
        \centering
        \includegraphics[width=1\columnwidth]{working_wec_phyiscal_diagram.pdf}
        \caption{WaveBot device diagram.}
        \label{fig:working_wec_phyiscal_diagram}
\end{figure}

\begin{table}[tb]
        \caption{Key physical parameters of the WaveBot.\rc{XX-sign conventions wrong in this diagram}}
        ...
        \hline
        Rigid body mass, $m$ [kg]                       & XX \\
        Hydrostatic stiffness, $k_h$ [N/m]              & XX \\
        Linear friction coefficient, $b_f$ [Ns/m]       & XX \\
        Linear to rot. gear ratio, $N$ [-]              & XX \\
        Shaft inertia, $J_d$ [kg\,m$^2$]                & XX \\
        Shaft friction, $b_d$ [Nm\,s/rad]               & XX \\
        Grounded shaft spring stiffness, $k_d$ [Nm/rad] & XX \\
        Torque constant, $k_\tau$ [Nm/A]                & XX \\
        Motor winding resistance, $R_w$ [$\Omega$]      & XX \\
        Motor winding inductance, $L_w$ [H]             & XX \\
        \hline
...
Vertical motion of the buoy can be expressed as a dynamic equation dependent on the PTO force ($F_{\textrm{pto}}$), the excitation force ($F_{\textrm{exc}}$), and the intrinsic impedance ($Z_i$) \cite{Falnes:2002aa}.
...
\noindent{}In addition to its gyration effect, the motor has a characteristic impedance, that captures the effects of winding resistance ($R_w$) and inductance ($L_w$), and relates the output current to the back EMF voltage and output voltage ($\forall_{\textrm{out}}$).
...
We may define a load impedance ($Z_\ell$) that represents how current is controlled by the motor controller.
\rc{XX-why negative?}
...
The WaveBot device shown in \figurename~\ref{fig:working_wec_phyiscal_diagram} and described in Section~\ref{sec:dynamic_and_kinematic_equations} can be represented with a network diagram as shown in \figurename~\ref{fig:working_paper_circuits}.
Various levels of abstraction and detail can be achieved in this style of diagram.
\figurename~\ref{fig:working_paper_circuits}a shows highest level of detail, whereas \figurename~\ref{fig:working_paper_circuits}b and \figurename~\ref{fig:working_paper_circuits}c increasingly combine elements to achieve a single two-port network to represent the PTO.
We will return to \figurename~\ref{fig:working_paper_circuits}d-f for later discussion.

\begin{figure}[tb]
        \centering
        \includegraphics[width=1\columnwidth]{working_paper_circuits.pdf}
        \caption{Circuit diagrams representing a wave energy converter. Points 1, 2, 3, and 4 are consistent throughout the different diagrams. \rc{XX-direction of $I_{out}$} \dg{XX: in b) Zd block is missing}}
        \label{fig:working_paper_circuits}
\end{figure}

% ------------------------------------------------------------------
\subsubsection{PTO impedance matrix}\label{sec:pto_impedance_matrix}

For the network shown in \figurename~\ref{fig:working_paper_circuits}c, we may write an impedance matrix to describe the two-port element representing the PTO.
...
\noindent{}Taken together, this gives 
 ...
 \noindent{}Note that \eqref{eq:pto_impedance} may also be arrived to by cascading the the one- and two-port elements shown in the more detailed diagram in \figurename~\ref{fig:working_paper_circuits}a.
...
\figurename~\ref{fig:working_paper_circuits}d gives the definition for the output impedance of the PTO ($Z_{\textrm{out}}$).
...
The PTO input port's impedance (described in \figurename~\ref{fig:working_paper_circuits}e) can be expressed as the ratio of effort ($F_{\textrm{pto}}$) and flow ($v$) on the input port.
...
Per \cite{Bacelli:2021aa}, to maximize power transferred to the load, we can apply impedance matching at the input and output ports of the PTO.
...
The complex power at the load is half the product of the effort ($Z_\ell I_{\textrm{out}}$) and complex conjugate flow ($I_{\textrm{out}}^*$) at the load \rc{XX-cite}.

\begin{equation}
\begin{aligned}
        \mathcal{S}_{\ell} 
        &=\forall_{\textrm{out}} I_{\textrm{out}}^* \\
...
\begin{align}
\begin{split}
\textrm{Active: } \mathcal{P}_{\ell} 
        &= \Re \left\{ \mathcal{S}_{\ell} \right\} \\
        &= XX
\end{split} \\[1em]
%
\begin{split}
\textrm{Reactive: } \mathcal{Q}_{\ell} 
        &= \Im \left\{ \mathcal{S}_{\ell} \right\} \\
        &= XX
\end{split} \\[1em]
%
\begin{split}
...
Returning to \figurename~\ref{fig:working_wec_phyiscal_diagram}, note that we have defined this load as the leads between the motor/generator and the motor controller.
Thus, as defined, the activate power at represents power into the motor controller.
...
For this reason, we may consider those losses separately and treat the motor controller as a static system (e.g., representing losses in the motor controller some function of motor torque and speed).

% ------------------------------------------------------------------
\subsubsection{Th\'{e}venin equivalent system}\label{sec:thevenin_equivalent_system}

\figurename~\ref{fig:working_paper_circuits}e shows the Th\'{e}venin equivalent network \cite{Thevenin:1883aa}, which has been considered by a number of authors with respect to WECs \cite{Bacelli:2021aa,Blanco:2019aa,Bubbar:2018aa,Lewis:2013aa}.
The Th\'{e}venin circuit defines an equivalent system containing only a source ($F_{\textrm{th}}$) and single impedance ($Z_{\textrm{th}}$).
Reviewing \figurename~\ref{fig:working_paper_circuits}e and our previous discussion about collapsing two-port elements to impedances, it becomes clear that the Th\'{e}venin equivalent impedance is equal to the PTO output impedance ($Z_{\textrm{th}} = Z_{\textrm{out}}$).
Similarly, the equivalent source can be determined by by considering a effort between points C and D in \figurename~\ref{fig:working_paper_circuits} when that part of the circuit is open (i.e., no connection between points C and D) such that flow through those points is zero ($I_{out}=0$).
...
Note that the bi-conjugate impedance matching condition prescribed in \eqref{eq:bi_conj_matching_out} is restated in \eqref{eq:Thevenin_impedance}  when we consider maximum power transfer for the Th\'{e}venin equivalent system shown in \figurename~\ref{fig:working_paper_circuits}e ($Z_\ell = Z_{\textrm{th}}^*$).
...
\begin{equation}
        ...
        = \left| \frac{ Z_{21} }{ Z_i + Z_{11} } \right| ^2 \frac{ | F_{\textrm{exc}} |^2 }{ 8 \Re \left\{ Z_{\textrm{th}} \right\} }
\end{equation}

As illustrated in \figurename~\ref{fig:working_paper_circuits}, we may represent the WEC system in many different ways (\figurename~\ref{fig:working_paper_circuits}a, b, c, and f are all equivalent).
These different representations have different utilities.
The Th\'{e}venin equivalent system in \figurename~\ref{fig:working_paper_circuits}f is the most simplified system and gives a clear condition for control design.
The single two-port representation of the PTO used in \figurename~\ref{fig:working_paper_circuits}c provides an additional impedance matching condition, thus more clearly expressing the co-design problem.
\figurename~\ref{fig:working_paper_circuits}b provides further detail by splitting the PTO into separate drive-train and electric machine two-port elements.
While not explored in this paper, a three-part impedance matching condition could be written for the representation shown in \figurename~\ref{fig:working_paper_circuits}b.

% ------------------------------------------------------------------
\section{Illustrative examples}\label{sec:illustrative_examples}
Many problems in WEC design can be considered through \eqref{eq:bi_conj_matching}.
...
In this scenario so very often examined in literature, mechanical power (the product of the input port effort and flow variables) is considered as the overall problem objective.
In this case, we shall define the \emph{mechanical} load impedance as

\begin{equation}
        Z_{\ell m} = \frac{F_{\textrm{pto}}}{v} .
\end{equation}

Reflections up to the input of the PTO may be minimized by satisfy a single well known conjugate impedance matching condition.
...
\noindent{}Not surprisingly, we see that \eqref{eq:mech_power_impedance_matching} is not dependent on the PTO impedance.
...
\begin{subequations}
        \begin{gather}
                Z_{\ell m} =Z_{\textrm{in}} = N^2 Z_d + \frac{k_\tau^2 N^2}{Z_\ell + Zw} \\
                Z_\ell = \frac{k_\tau^2 N^2}{Z_{\ell m} - N^2 Z_d} - Z_w
        \end{gather}
        \label{eq:Zl_Zlm}
\end{subequations}

\figurename~\ref{fig:working_paper_load_impedance_for_mech_power} uses \eqref{eq:Zl_Zlm} to show the resulting load impedances for two cases:
...
\begin{figure}[tb]
        \centering
        \includegraphics[width=1\columnwidth]{working_paper_load_impedance_for_mech_power.pdf}
        ...
        \label{fig:working_paper_load_impedance_for_mech_power}
\end{figure}

\noindent{}From \figurename~\ref{fig:working_paper_load_impedance_for_mech_power}, we can see that the mechanical maximizing controller looks very different from the controller that maximizes electrical power.

It is also helpful to define the mechanical power at the input of the PTO.
...
\begin{aligned}
        ...
        &= Z_{\ell m} v v^* \\
        &= Z_{\ell m} | v |^2 \\
        &= Z_{\ell m} \left| \frac{F_{\textrm{exc}}}{Z_i - Z_{\ell m}} \right|^2
\end{aligned}
\end{equation}

As with the power at the load, we may similarly quantify the active, reactive, and apparent mechanical power at the PTO input as shown in \eqref{eq:load_power_types}.
...
% \rc{
% \begin{itemize}
%         \item XX-Compare $Z_\ell$ vs. $Z_{\textrm{out}}$ for different realistic options of $Z_{\ell m}$
%         \begin{itemize}
%                 \item $Z_{\ell m} = Z_i^*$
%                 \item $Z_{\ell m} = \Re \{ Z_i^* \}$
%                 \item $Z_{\textrm{out}} = Z_\ell^*$ (the true optimal controller)
%         \end{itemize}
%         \item $P_m$ and $P_e$ for each of the above scenarios
%         \item XX-do the elec. and mech. maximizing controllers look more alike as you re-design the hardware (e.g., add negative spring)?
% \end{itemize}
% }
...
Consider, for example, \figurename~\ref{fig:working_paper_in_and_out_impedances}, where we separately investigate the effects of adding a negative stiffness element\footnote{The addition of a negative stiffness has been applied by CorPower in the form a hydraulic assembly \cite{Todalshaug:2016aa} and also by Sandia Labs via a tunable magnetic spring \cite{Forbush:2024aa}.} to the drive train (\figurename~\ref{fig:working_paper_in_and_out_impedances_spring}) and adding inertia to the drive train (\figurename~\ref{fig:working_paper_in_and_out_impedances_inertia}).
...
\begin{figure}
     ...
     \begin{subfigure}[b]{1\columnwidth}
         \centering
         \includegraphics[width=\textwidth]{working_paper_in_and_out_impedances_spring.pdf}
         \caption{Varying levels of negative stiffness on shaft ($K_d$)}
         \label{fig:working_paper_in_and_out_impedances_spring}
     \end{subfigure}
     \\
     \begin{subfigure}[b]{1\columnwidth}
         \centering
         \includegraphics[width=\textwidth]{working_paper_in_and_out_impedances_inertia.pdf}
         \caption{Varying levels of shaft inertia ($I_d$)}
         \label{fig:working_paper_in_and_out_impedances_inertia}
     \end{subfigure}
     ...
     \label{fig:working_paper_in_and_out_impedances}
\end{figure}
...
It should be noted that the range of values for $K_d$ and $I_d$ shown in \figurename~\ref{fig:working_paper_in_and_out_impedances} is somewhat arbitrary and intended only to illustrate trends -- one cannot directly compare the effects of doubling the shaft inertia with doubling the negative spring stiffness in terms of an engineering decision.
With this in mind, we can see that, for this specific example, adding shaft inertia (\figurename~\ref{fig:working_paper_in_and_out_impedances_inertia}) appears to adversely affect the shape of the input impedance (both in terms of the real and imaginary parts), thus narrowing the bandwidth over which a good matching with the intrinsic impedance is achieved, whereas adding a negative spring (\figurename~\ref{fig:working_paper_in_and_out_impedances_inertia}) appears to have the opposite affect, generally increasing the bandwidth over which good matching with the intrinsic impedance occurs.


While based on \eqref{eq:drive_train_impedance} we might expect changes to $K_d$ and $I_d$ to only affect the imaginary part of the impedances, we may see by expanding \eqref{eq:pto_output_port_impedance} and \eqref{eq:pto_input_port_impedance} that these terms do indeed effect the real part of the impedances as shown in \figurename~\ref{fig:working_paper_in_and_out_impedances}.
...
% We may now conceptualize the ``wave-to-wire'' WEC system as the network shown in \figurename~\ref{fig:working_paper_circuits}.
% On the far left hand side of \figurename~\ref{fig:working_paper_circuits}, we have the oscillating effort $F_{\textrm{exc}}$.
...
% We should also remember that, in practice, we must restrict our focus to a finite bandwidth of interest, both because of the band-limited nature of ocean waves (see, e.g., \cite{Coe2020a}) and because a practical design that balances cost and revenue likely needs to be discerning about the added value of improved performance.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{dummy.pdf}}]{Ryan G. Coe}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{dummy.pdf}}]{Giorgio Bacelli} received his master's degree from Università Politecnica delle Marche and his PhD from the National University of Ireland, Maynooth. 
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{dummy.pdf}}]{Dominic Forbush}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{dummy.pdf}}]{Daniel T. Gaebele}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{dummy.pdf}}]{Alicia Keow}
\rc{XX\dots{}}
\end{IEEEbiography}
