...
The WaveBot device shown in \figurename~\ref{fig:wec_as_multiport_phyiscal_diagram} and described in Section~\ref{sec:dynamic_and_kinematic_equations} can be represented with a network diagram as shown in \figurename~\ref{fig:wec_as_multiport_circuits}.
...
\noindent{}Taken together, this gives 
 ...
 \noindent{}Note that \eqref{eq:pto_impedance} may also be arrived to by cascading the the one- and two-port elements shown in the more detailed diagram in \figurename~\ref{fig:wec_as_multiport_circuits}a.
...
\figurename~\ref{fig:wec_as_multiport_circuits}d gives the definition for the output impedance of the PTO ($Z_{\textrm{out}}$).
...
The PTO input port's impedance (described in \figurename~\ref{fig:wec_as_multiport_circuits}e) can be expressed as the ratio of effort ($F_{\textrm{pto}}$) and flow ($v$) on the input port.
...
\figurename~\ref{fig:wec_as_multiport_circuits}e shows the Th\'{e}venin equivalent network \cite{Thevenin:1883aa}, which has been considered by a number of authors with respect to WECs \cite{Bacelli:2021aa,Blanco:2019aa,Bubbar:2018aa,Lewis:2013aa}.
...
Similarly, the equivalent source can be determined by by considering a effort between points C and D in \figurename~\ref{fig:wec_as_multiport_circuits} when that part of the circuit is open (i.e., no connection between points C and D) such that flow through those points is zero ($I_{out}=0$).
...
Note that the bi-conjugate impedance matching condition prescribed in \eqref{eq:bi_conj_matching_out} is restated in \eqref{eq:Thevenin_impedance}  when we consider maximum power transfer for the Th\'{e}venin equivalent system shown in \figurename~\ref{fig:wec_as_multiport_circuits}e ($Z_\ell = Z_{\textrm{th}}^*$).
...
The Th\'{e}venin equivalent system in \figurename~\ref{fig:wec_as_multiport_circuits}f is the most simplified system and gives a clear condition for control design.
The single two-port representation of the PTO used in \figurename~\ref{fig:wec_as_multiport_circuits}c provides an additional impedance matching condition, thus more clearly expressing the co-design problem.
