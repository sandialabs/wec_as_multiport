function outHydro = fix_bem_spikes(filepath)

addpath(genpath('.'))

outHydro=badBemioFix_fcn({filepath}, ...
    'CAPYTAINE',[],[1,1;3,3;5,5]);
...
if isempty(deSpike) % if the third argument is empty will use some default values
    deSpike = struct();
    deSpike.negThresh = 1e-4; % the threshold below which negative damping will be removed
    deSpike.N = 50; % will loop the despiking procedure N time before filtering
    ...
    deSpike.Threshold.B = 1e-3; % damping
    ...
    deSpike.MinPeakDistance.ExRe = 1;
    deSpike.MinPeakDistance.ExIm = 1;
    ...
    % IRF parameters
    deSpike.IRF.wMin = 0.06;
    deSpike.IRF.wMax = 9.9;
...
writeBEMIOH5(hydro, fullfile(fileparts(fileName{1}),[hydro.file,'.h5']));
plotBEMIO(hydro);
